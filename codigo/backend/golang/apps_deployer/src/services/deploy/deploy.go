...
func Deploy(name, image string, port nat.Port, envs []structs.Env) (err error) {
	...
	if err != nil {
		deployment, err = appsV1Client.Deployments(namespace).Update(
			ctx, deploymentSpec, metaV1.UpdateOptions{})

		if err != nil {
			errorMessage := fmt.Sprintf("failed to create/update Kubernetes Deployment %s", name)
